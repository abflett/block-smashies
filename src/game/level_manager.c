...
static void load_level(void)
{
    ...
    const JSON_Array *level_array = json_value_get_array(root_value);
    const size_t level_count = json_array_get_count(level_array);

    // Iterate over the levels to find the one that matches the current mission and operation
    for (size_t i = 0; i < level_count; i++)
    {
        const JSON_Object *level_object = json_array_get_object(level_array, i);

        const int operation = (int)json_object_get_number(level_object, "operation");
        const int mission = (int)json_object_get_number(level_object, "mission");
        ...
        if (operation == manager.context->game_data->operation && mission == manager.context->game_data->mission)
        {
            ...
            const JSON_Array *grid_array = json_object_get_array(level_object, "grid");
...
static void create_level(void)
{
    ...
    const int brick_row = 12;
    const int brick_column = 12;

    // Define spacing for rows and columns
    const float row_spacing = 18.0f; // Horizontal spacing (width of the brick + any gap)
    const float col_spacing = 8.0f;  // Vertical spacing (height of the brick + any gap)
    ...
    for (int col = 0; col < brick_row; col++)
    {
        ...
        for (int row = 0; row < brick_column; row++)
        {
            ...
            const int brick_type = manager.level_data.data[col * brick_column + row];
            ...
            if (brick_type != 0)
            {
                ...
                const b2Vec2 position = {
                    (float)row * row_spacing + settings.game.play_area.x + 10,                               // X position
                    settings.game.target_size.y - ((float)col * col_spacing + settings.game.play_area.y + 5) // Y position (adjust for screen coords)
                };
