...
static void update(struct GameDialog *game_dialog, float delta_time)
{
...
static void render(struct GameDialog *game_dialog)
{
    if (!game_dialog || !game_dialog->active || !game_dialog->font)
        return; // Error checking

    Vector2 pos = game_dialog->position;
    Vector2 size = {200.f, 60.f}; // Size of the dialog box
    ...
    while (line)
    {
        ...
        float line_width = MeasureTextEx(*game_dialog->font, line, line_height, 0.0f).x;
    ...
    Vector2 btn_size = {50.f, 10.f};
    float btn_space = 20.f;

    // Calculate button positions
    const char *btns[] = {"No", "Yes"};
    Color bkg = WHITE, txt = BLACK;

    // Adjust button positions based on the number of lines drawn
    for (int i = 0; i < 2; ++i)
    {
        float x_pos = pos.x + (size.x - ((btn_size.x * 2) + btn_space)) * 0.5f + (btn_size.x + btn_space) * i;
        DrawRectangle((int)x_pos, (int)(pos.y + 45 + (line_height * (msg_pos.y / line_height))), (int)btn_size.x, (int)btn_size.y, bkg);
        Vector2 text_pos = {x_pos + (btn_size.x - MeasureTextEx(*game_dialog->font, btns[i], 7, 0).x) * 0.5f, pos.y + 46 + (line_height * (msg_pos.y / line_height))};
        DrawTextEx(*game_dialog->font, btns[i], text_pos, 7, 0.0f, txt);
    }
}
...
GameDialog *create_game_dialog(const char *message, Vector2 position)
{
    ...
    game_dialog->active = true;
    game_dialog->message = strdup(message); // Use strdup to allocate and copy the message
