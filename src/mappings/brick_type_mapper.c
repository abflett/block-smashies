...
static const char *brick_type_to_subtexture_id_func(const int brick_type, const int index)
{
    ...
    const size_t brick_type_mod = (brick_type < 0) ? 0 : (brick_type % kv_size(mapper.subtexture_sets));

    // Ensure index is non-negative, otherwise default to 0, and limit it to 0-3
    const size_t index_mod = (index < 0) ? 0 : (index % 4);
...
static void cleanup_brick_type_mapper(void)
{
    ...
    {
        const BrickSubtextureIdsSet set = kv_A(mapper.subtexture_sets, i);
        for (int j = 0; j < 4; ++j)
        {
...
BrickTypeMapper *create_brick_type_mapper(const JSON_Object *root_object)
{
    ...
    const JSON_Array *brick_type_array = json_object_get_array(root_object, "brick_types");

    for (size_t i = 0; i < json_array_get_count(brick_type_array); i++)
    {
        const JSON_Object *brick_type_obj = json_array_get_object(brick_type_array, i);
        const char *animation_id = json_object_get_string(brick_type_obj, "animation");

        const JSON_Array *subtexture_array = json_object_get_array(brick_type_obj, "subtextures");
